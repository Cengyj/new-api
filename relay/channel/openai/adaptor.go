	...
	case relayconstant.RelayModeImagesEdits:
		...
		if mf != nil && mf.File != nil {
			...
			for i, fileHeader := range imageFiles {
				...
				// grok2api requires image[] even for a single Grok image-edit
				// reference image, so keep the compatibility branch model-scoped.
				fieldName := "image"
				if len(imageFiles) > 1 || isGrokImageEditModel(request.Model) {
					fieldName = "image[]"
				}
...
func isGrokImageEditModel(model string) bool {
	switch model {
	case "grok-imagine-image-edit", "grok-imagine-image", "grok-imagine-image-pro":
		return true
	default:
		return false
	}
