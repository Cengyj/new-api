...
const SCENE_PREF_STORAGE_KEY = 'ai_creation_scene_preferences';
const SUPPORTED_SCENES = new Set(['chat', 'image', 'video']);

export const loadScenePreferences = () => {
  try {
    const raw = localStorage.getItem(SCENE_PREF_STORAGE_KEY);
    if (!raw) return {};
    const parsed = JSON.parse(raw);
    if (parsed && typeof parsed === 'object') return parsed;
  } catch (error) {
    console.error('加载场景偏好失败:', error);
  }
  return {};
};

export const loadScenePreference = (scene) => {
  if (!SUPPORTED_SCENES.has(scene)) return {};
  const all = loadScenePreferences();
  const pref = all[scene];
  return pref && typeof pref === 'object' ? pref : {};
};

export const saveScenePreference = (scene, partial) => {
  if (!SUPPORTED_SCENES.has(scene) || !partial) return;
  try {
    const all = loadScenePreferences();
    const current =
      all[scene] && typeof all[scene] === 'object' ? { ...all[scene] } : {};
    ...
    all[scene] = current;
    localStorage.setItem(SCENE_PREF_STORAGE_KEY, JSON.stringify(all));
  } catch (error) {
    console.error('保存场景偏好失败:', error);
  }
};

export const clearScenePreferences = () => {
  try {
    localStorage.removeItem(SCENE_PREF_STORAGE_KEY);
  } catch (error) {
    console.error('清除场景偏好失败:', error);
  }
};
