...
const buildChatSidebarModules = (modules = {}) => {
  const { aiCreation, ...rest } = modules;
  const legacyValue = typeof aiCreation === 'boolean' ? aiCreation : true;

  return {
    enabled: true,
    aiCreationChat: legacyValue,
    aiCreationImage: legacyValue,
    aiCreationVideo: legacyValue,
    playground: true,
    chat: true,
    ...rest,
  };
      ...
      try {
        ...
        if (userRes.data.success && userRes.data.data.sidebar_modules) {
          ...
          setSidebarModulesUser({
            chat: buildChatSidebarModules(userConf.chat),
            console: {
              enabled: true,
              detail: true,
              token: true,
      ...
      modules: [
        {
          key: 'aiCreationChat',
          title: t('对话'),
          description: t('简洁对话工作台，可快速切换分组和模型'),
        },
        {
          key: 'aiCreationImage',
          title: t('生图'),
